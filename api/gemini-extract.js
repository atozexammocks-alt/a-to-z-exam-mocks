...
const model=GEMINI_MODELS[geminiIndex];
...
try{
 ...
 try{
  const requestBody=model
   ? {contents:[{role:'user',parts}],generationConfig:{responseMimeType:'application/json',responseSchema:schema,maxOutputTokens:4096}}
   : {model:OPENROUTER_MODEL,messages:[{role:'user',content:parts.map(p=>p.inline_data?{type:'image_url',image_url:{url:'data:'+p.inline_data.mime_type+';base64,'+p.inline_data.data}}:{type:'text',text:p.text||''})}],response_format:{type:'json_object'},max_tokens:4096};
  const headers=model
 ...
 if(mode==='scan_questions')return send(res,200,{questions:cleanQuestions(parsed?.questions),provider,model:model||OPENROUTER_MODEL});
 return send(res,200,{items:cleanQuestions(parsed),provider,model:model||OPENROUTER_MODEL});
